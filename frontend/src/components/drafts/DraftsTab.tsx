...
export default function DraftsTab() {
  ...
  function handleDownload() {
    ...
    a.href = url
    a.download = `${draftType.replace(/\s+/g, '_')}_Draft.txt`
    a.click()
  ...
  function handlePrint() {
    ...
    if (printWindow) {
      printWindow.document.write(`<pre style="font-family: serif; font-size: 14pt; white-space: pre-wrap; padding: 40px;">${draft}</pre>`)
  ...
  async function handleSaveToDocuments() {
    if (!selectedCaseId || !draft) return
    setIsSaving(true)
    try {
      // Import documentsApi dynamically or it should already be available in api.ts
      const { documentsApi } = await import('@/lib/api')
      const blob = new Blob([draft], { type: 'text/plain;charset=utf-8' })
      const file = new File([blob], `${draftType.replace(/\s+/g, '_')}_Draft.txt`, { type: 'text/plain' })
      await documentsApi.upload(selectedCaseId, file, 'Legal Draft')
      alert('Draft saved to Case Documents successfully!')
    } catch (err: any) {
      ...
      <motion.div variants={itemVariants} style={{ display: 'flex', alignItems: 'center', padding: '16px 24px', borderBottom: '1px solid rgba(0,0,0,0.05)', background: 'rgba(255,255,255,0.3)', flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          >
            ...
            {generating ? 'Drafting...' : draft ? 'Regenerate' : 'Generate'}
          </MotionButton>
        </div>
      </motion.div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '35% 1fr', gap: 20, padding: '24px', flex: 1, overflow: 'hidden' }}>
        ...
        <motion.div variants={itemVariants} style={{ height: '100%', minHeight: 0 }}>
          <MotionCard style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', height: '100%', background: 'rgba(255,255,255,0.4)', borderRadius: 24, border: '1px solid rgba(0,0,0,0.05)' }}>
            <div style={{ padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid rgba(0,0,0,0.05)', background: 'rgba(255,255,255,0.3)', flexWrap: 'wrap' }}>
              ...
              {draft && (
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={handlePrint} style={{ padding: '6px 12px', fontSize: 12, borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', color: '#475569', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600 }}>
                    <i className="ti ti-printer" style={{ fontSize: 14 }} /> Print
                  </button>
                  <button onClick={handleDownload} style={{ padding: '6px 12px', fontSize: 12, borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', color: '#475569', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600 }}>
                    <i className="ti ti-download" style={{ fontSize: 14 }} /> .TXT
                  </button>
                  <button onClick={handleCopy} style={{ padding: '6px 12px', fontSize: 12, borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', color: '#475569', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600 }}>
                    <i className="ti ti-copy" style={{ fontSize: 14 }} /> Copy
                  </button>
                  <button onClick={handleSaveToDocuments} disabled={isSaving} style={{ padding: '6px 12px', fontSize: 12, borderRadius: 8, border: 'none', background: '#10b981', cursor: isSaving ? 'not-allowed' : 'pointer', color: 'white', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, boxShadow: '0 2px 8px rgba(16, 185, 129, 0.3)' }}>
                    <i className="ti ti-device-floppy" style={{ fontSize: 14 }} /> {isSaving ? 'Saving...' : 'Save to Case'}
                  </button>
                </div>
              )}
            </div>
            ...
            <div style={{ flex: 1, padding: '24px', overflowY: 'auto' }}>
              {generating && (
                <div style={{ textAlign: 'center', padding: 40, color: '#64748b', fontSize: 13 }}>
                  <div style={{ marginBottom: 12 }}>⚖️ Drafting {draftType}...</div>
              ...
              {!generating && draft && (
                <textarea 
                  ...
                  style={{ 
                    width: '100%',
                    background: '#ffffff', borderRadius: 16, padding: '40px 48px', minHeight: 600, 
                    fontFamily: 'Times New Roman, serif', color: '#000', fontSize: 15, lineHeight: 1.8, 
                    boxShadow: '0 4px 20px rgba(0,0,0,0.08)', outline: 'none', border: '1px solid #e2e8f0',
                    resize: 'none', height: '800px'
                  }}
                />
              )}
            </div>
